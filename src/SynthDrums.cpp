...
struct SynthDrums_Widget : ModuleWidget {
    ...
    SynthDrums_Widget(SynthDrums *module) {
       ...
       for (ch = 0; ch < nCHANNELS; ch++) {
          ...
          // wave select buttons
          m_pButtonWaveSelect[ch] = new MyLEDButtonStrip(x, y, 11, 11, 5, 8.0, 5, false, DWRGB(180, 180, 180),
                                                         DWRGB(255, 255, 0), MyLEDButtonStrip::TYPE_EXCLUSIVE, ch,
                                                         module, SynthDrums_WaveSelect);
...
void SynthDrums::process(const ProcessArgs &args) {
   ...
   outputs[OUTPUT_AUDIO + 0].setVoltage(
           GetAudio(0) * AUDIO_MAX * clamp((inputs[IN_LEVEL + 0].getVoltage() / CV_MAX10), 0.0f, 1.0f));
   outputs[OUTPUT_AUDIO + 1].setVoltage(
           GetAudio(1) * AUDIO_MAX * clamp((inputs[IN_LEVEL + 1].getVoltage() / CV_MAX10), 0.0f, 1.0f));
   outputs[OUTPUT_AUDIO + 2].setVoltage(
           GetAudio(2) * AUDIO_MAX * clamp((inputs[IN_LEVEL + 2].getVoltage() / CV_MAX10), 0.0f, 1.0f));
}
